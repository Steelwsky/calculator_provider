...
class CalcController {
  ...
  void onNumber(String input) {
    ...
    if (state.value.operator == null) {
      state.value = CalcState(num1: state.value.num1.replaceAll('0', '') + input);
      printInfo();
    } else {
      state.value = CalcState(num1: state.value.num1, num2: state.value.num2 + input, operator: state.value.operator);
    }
  }
